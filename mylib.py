...
TABLESPACE_NAME=r'E:\deve\mysql\mysqlDate\test\t1.ibd\t1.ibd'
...
def get_innodb_page_type(myargv):
    ...
    for type in ret:
        #print("DEBUG TYPE:%s" % type)
        print ("%s: %s"%(innodb_page_type[type],ret[type]))
    print ('zero_count %s' % zero_count)
    print ('non_zero_index %s' % non_zero_index)
###